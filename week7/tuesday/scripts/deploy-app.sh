...
#
# Optional:
#   BLUE_PORT        - Port for the blue service (default: 3000)
#   GREEN_PORT       - Port for the green service (default: 3001)
...
log_fail() {
  ...
  validate_artifact() {
    ...
    if [ -f "${dest}.sha256" ]; then
      (
        cd /opt/kijanikiosk/releases
        sha256sum -c "kk-api-${APP_VERSION}.tar.gz.sha256"
      ) || {
   ...
   [ -f "${staging}/kk-api-${APP_VERSION}/server.js" ] || {
  ...
  deploy_artifact() {
    ...
    # DEPLOY_ENV is either "blue" (port 3000) or "green" (port 3001)
    ...
    cp -r "${staging}/kk-api-${APP_VERSION}" "${target_dir}/new_version"
